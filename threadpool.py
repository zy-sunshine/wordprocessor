...
class ThreadPool(threading.Thread):
    ...
    def __init__(self, num_workers, q_size=0, resq_size=0, poll_timeout=2, **kwds):
    ...
    def poll(self, block=True, timeout=1):
        while not self._dismissed.isSet():
            try:
                if not block:
                    time.sleep(timeout)
                #if not self.workRequests and self._dismissed.isSet():
    ...
    def join(self, timeout=None):
        ...
        self.dismiss()
        super(ThreadPool, self).join(timeout)

    # TODO: implement terminate, to drop rest request and self.cloes().
    #def terminate()
...
if __name__ == '__main__':
    ...
    data = [random.randint(1,10) for i in range(3)]
