...
##            errorfile=self.blog.theme.error
    ...
    def render(self, template, params={}, mimetype=None, status=None,
            content_type=None):
        """
        Helper method to render the appropriate template
        """
        params.update(self.template_vals)
        #template = get_template_uri(self.app_context, template)
    ...
    def paramlong(self, name, default=0):
        value = self.param(name)
        try:
           return long(value)
        except:
...
### Adapt to micolog
class BaseRequestHandler(SiteRequestHandler):
    def __init__(self):
        SiteRequestHandler.__init__(self)
    def initialize(self, request):
        SiteRequestHandler.initialize(self, request)
